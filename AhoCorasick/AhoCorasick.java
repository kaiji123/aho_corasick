...
static int buildMatchingMachine(String arr[], int k)
{
    ...
    for(int i = 0; i < k; ++i)
    {
        ...
        System.out.println("cur "+ currentState );
    ...
    while (!q.isEmpty())
    {
        ...
        for(int ch = 0; ch < MAXC; ++ch)
        {
            ...
            if (g[state][ch] != -1)
            {
                ...
                // state.
                while (g[failure][ch] == -1) // since the first root nodes are changed to 0 the loop will eventually end
                      failure = f[failure];
                ...
                //System.out.println(out[g[state][ch]]);
    ...
    for(int i = 0; i < text.length(); ++i)
    {
        ...
        for(int j = 0; j < k; ++j)
        {
            if ((out[currentState] & (1 << j)) > 0)
            {
            	...
            	//(1 << j) is for each word since we inserted them using out[currentState] |= (1 << i);
            	// also if a subword exists it will that subword since we merged them using out[g[state][ch]] |= out[failure];
            	System.out.println(out[currentState]);
